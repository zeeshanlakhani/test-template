...
- [test-template-wasm](https://github.com/zeeshanlakhani/test-template/tree/main/test-template-wasm)
...
- Build a multi-plaform Docker image via [buildx][buildx] (from top-level):
  ...
  ```console
  docker buildx build --platform=linux/amd64,linux/arm64 --file docker/Dockerfile -t test-template --progress=plain .
  ```
