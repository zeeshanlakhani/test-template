...
`test-template` contains a file for [configuration settings](./config/settings.toml),
...
[integration test](./test-template/tests/integration_test.rs),
...
*Note*: Our [logging middleware](./test-template/src/middleware/logging.rs)
...
[middleware/client.metrics.rs](./test-template/src/middleware/client/metrics.rs).
For the `axum` webserver itself, metrics are derived via
[middleware/metrics.rs](./test-template/src/middleware/metrics.rs).
...
For logs, traces, and metrics, `test-template` utilizes several log levels
...
to capture everything flowing through `test-template` before events are
diffracted to their respective log levels--this way it is possible for
`test-template` to maintain contextual trace information throughout the
...
`test-template` implements hooks around the creation and closing of spans
...
setting OTEL span information when `test-template` application
